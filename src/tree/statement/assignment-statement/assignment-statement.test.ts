...
import { AssignmentStatementTree, parseCode } from '@xon/ast';
...
test('simple assign', () => {
    ...
    const result = getStatementTranslator(tree).translate();
...
test('several assign', () => {
    ...
    const tree = parseCode(code, AssignmentStatementTree);
    const result = getStatementTranslator(tree).translate();
    expect(result).toBe(`___id_0 = [5, *[1, 2, 3]]
a = ___id_0[0]
b = ___id_0[2]
c = ___id_0[3]`);
});
...
test('spread assign', () => {
    ...
    const tree = parseCode(code, AssignmentStatementTree);
    const result = getStatementTranslator(tree).translate();
    expect(result).toBe(`___id_0 = [1, 2, 3]
a = ___id_0[0]
c = ___id_0[-1]
b = ___id_0[1:-1]`);
});
